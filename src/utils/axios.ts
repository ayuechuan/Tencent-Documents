...
export class AxiosErrorModel<T = void> {
  ...
  static async callbackArray_body<CT>(
    promise: Promise<AxiosResponse<CT>>,
  ): Promise<[null, AxiosErrorModelWithHeder<CT>] | [AxiosErrorModelWithHeder, null]> {
    return promise
...
class Instace {
  ...
  public upload<T>(data: Upload) {
    ...
    return AxiosErrorModel.callbackArray<T>(
      this.instace.post(url, formData, {
  ...
  public async uploadStream<T>(data: UploadStream) {
    ...
    return AxiosErrorModel.callbackArray<T>(
      this.instace.post(url, fileArrayBuffer, {
  ...
  public axiosDownload(params: AxiosDownload): Promise<{ fileName: string }> {
    ...
    return new Promise((resolve, reject) => {
      AxiosErrorModel.callbackArray_body(
        this.instace.get<Blob>(url, {
          params: data,
          responseType: 'blob',
          onDownloadProgress,
...
export class FileServer extends OpenApi {
  ...
  public static serach(params: any) {
    return this.register('/socketServer').getResponseArray<{ name: string }>('/public/v1/goods/search', params)
  }
}

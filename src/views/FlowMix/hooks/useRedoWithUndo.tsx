...
export function useRedoWithUndo() {
  ...
  const map = new Map([
    [
      ...
      (stackItem: EditorHistory) => { setNodes((nds) => nds.concat(stackItem.addNode!)); }
    ],
    [
      (stackItem: EditorHistory) => Reflect.has(stackItem, 'deleteNode'),
      (stackItem: EditorHistory) => { setNodes((nds) => nds.filter((node) => node.id !== stackItem.deleteNode!.id)); }
    ],
    [
    ...
    [
      ...
      (stackItem: EditorHistory) => { setNodes((nds) => nds.concat(stackItem.addNodes!)); }
    ],
    [
      (stackItem: EditorHistory) => Reflect.has(stackItem, 'deleteNodes'),
      (stackItem: EditorHistory) => { setNodes((nds) => nds.filter((node) => !stackItem.deleteNodes!.some((nodeItem) => nodeItem.id === node.id))); }
    ],
    [

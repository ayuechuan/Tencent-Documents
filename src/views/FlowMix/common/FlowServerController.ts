  ...
  private executeHistoryStack(stack: EditorHistorys) {
    const map = new Map([
      [
        ...
        (stackItem: EditorHistory) => { this.addNode(stackItem.addNode!) }
      ],
      [
        (stackItem: EditorHistory) => Reflect.has(stackItem, 'deleteNode'),
        (stackItem: EditorHistory) => { this.deleteNode(stackItem.deleteNode!) }
      ],
      [
        (stackItem: EditorHistory) => Reflect.has(stackItem, 'addEdge'),
        (stackItem: EditorHistory) => { this.addEdge(stackItem.addEdge!) }
      ],
      [
        (stackItem: EditorHistory) => Reflect.has(stackItem, 'deleteEdge'),
        (stackItem: EditorHistory) => { this.deleteEdge(stackItem.deleteEdge!) }
      ],
      [
        (stackItem: EditorHistory) => Reflect.has(stackItem, 'addNodes'),
        (stackItem: EditorHistory) => { this.addNodes(stackItem.addNodes!) }
      ],
      [
        (stackItem: EditorHistory) => Reflect.has(stackItem, 'deleteNodes'),
        (stackItem: EditorHistory) => { this.deleteNodes(stackItem.deleteNodes!) }
      ],
      [
        (stackItem: EditorHistory) => Reflect.has(stackItem, 'addEdges'),
        (stackItem: EditorHistory) => { this.addEdges(stackItem.addEdges!) }
      ],
      [
        (stackItem: EditorHistory) => Reflect.has(stackItem, 'deleteEdges'),
        (stackItem: EditorHistory) => { this.deleteEdges(stackItem.deleteEdges!) }
      ],
    ]);

    stack.forEach((stackItem) => {
      map.forEach((fn, key) => {
        if (key(stackItem)) { fn(stackItem) }
      })
    })
  }
